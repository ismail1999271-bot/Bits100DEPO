...
def attribute_and_store(inp: AttributionInput, memory: ResearchMemory) -> SignalAttribution:
    ...
    contributions = {key: round(value / total * inp.score, 6) if total else 0.0 for key, value in inp.feature_scores.items()}
    attribution = SignalAttribution(inp.signal_id, inp.symbol, inp.timestamp, inp.score, None, None, contributions, inp.source_ids)
...
def close_signal(memory: ResearchMemory, signal_id: str, outcome_return: float, event_threshold: float = 0.10) -> bool:
    ...
    return memory.close_signal(signal_id, outcome_return, event_threshold)
